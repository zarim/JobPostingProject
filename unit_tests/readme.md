...
## Results and Expanded Results
The Javascript files for the candidate-results, expanded-candidate, jobSearch, and expanded-jobs are used to pull data from the database, create HTML code for them, and append the newly created HTML code to the respective HTML files. When trying to write unit tests for this functionality there was issues with accessing Firebase and figuring out how to test the actual results of the functions. To account for this, videos have been added to the folders to show that data is actually being pulled from the database and appended to the HTML pages.